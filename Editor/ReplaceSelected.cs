...
namespace AdeelRiaz.Editor
{
    ...
    private const float HEIGHT = 250f;
        ...
        private static Object _sourcePrefab;

        private Texture _refreshIcon;
        private void OnEnable()
        {
            _refreshIcon = EditorGUIUtility.IconContent("Refresh").image;
        ...
        {
            get => EditorPrefs.GetBool(PlayerSettings.productName + "_REPLACE_KeepStaticFlags", false);
        ...
        private void OnGUI()
        {
            ...
            using (new EditorGUI.DisabledGroupScope(sourceObject == null))
            {
                ...
                if (GUILayout.Button(new GUIContent(" Replace " + Selection.gameObjects.Length + " GameObject" + (Selection.gameObjects.Length > 1 ? "s" : ""), _refreshIcon), GUILayout.Height(25f)))
                {
        ...
        private void ReplaceCurrentSelection()
        {
            ...
            _sourcePrefab = PrefabUtility.GetPrefabAssetType(sourceObject) == PrefabAssetType.Variant ? sourceObject : PrefabUtility.GetCorrespondingObjectFromOriginalSource(sourceObject);

            //Model prefabs don't count
            var isPrefab = _sourcePrefab;
        ...
        private static void Replace(Object source, GameObject target, bool sourceIsPrefab)
        {
            ...
            if (sourceIsPrefab)
            {
                newObj = PrefabUtility.InstantiatePrefab(_sourcePrefab, target.scene) as GameObject;
